...
#' @importFrom dplyr %>% across all_of bind_cols bind_rows case_when distinct everything if_else inner_join filter full_join group_by lag lead left_join mutate n pull reframe row_number select tibble tribble ungroup slice matches
#' @importFrom ggplot2 aes arrow facet_grid facet_wrap geom_area geom_label geom_line geom_point geom_ribbon geom_segment geom_step geom_text geom_tile geom_vline ggplot ggtitle labs scale_colour_manual scale_fill_brewer scale_fill_continuous scale_fill_gradient2 scale_size theme theme_bw theme_void geom_col scale_y_continuous element_text ylab xlab ylim
...
#' @importFrom stats na.omit prop.test quantile rbeta rbinom runif setNames qbeta
